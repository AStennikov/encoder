...
uint16_t calculateSensorPosition(int16_t* interpolatedValues) {
	for(uint32_t diffPos=0; diffPos<MAGNETIC_PATTERN_LENGTH; ++diffPos) {			// iterating through angles in magnetic pattern
		...
		for(uint32_t valTau=0; valTau<INTERPOLATED_SENSOR_ARRAY_LENGTH; ++valTau){	// adding up difference between magnetic pattern and interpolated values
			int32_t difference = (int32_t) magneticPattern[(diffPos + valTau) % MAGNETIC_PATTERN_LENGTH] - (int32_t) interpolatedValues[valTau];
			diff[diffPos] += difference*difference;
		}
	}

	// finding minimum in the difference vector
	int32_t minimum = 0x7FFFFFFF;
	uint16_t minimumPos = 0;
	for(uint32_t diffPos=0; diffPos<MAGNETIC_PATTERN_LENGTH; ++diffPos) {
	...
	/*def diff(pattern, signal):
	    result = np.zeros(len(pattern))

	    for tau in range(len(result)):
	        for i in range(len(signal)):
	            result[tau] = result[tau] + pow(pattern[(tau + i) % len(pattern)] - signal[i], 2)
